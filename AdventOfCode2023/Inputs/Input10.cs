...
public class Input10 : AbstractInput<(Tuple<int, int> startingPosition, ImmutableDictionary<Tuple<int, int>, Day10.Pipe> pipes), (Tuple<int, int> startingPosition, ImmutableDictionary<Tuple<int, int>, Day10.Pipe> pipes)>
{
    protected override (Tuple<int, int> startingPosition, ImmutableDictionary<Tuple<int, int>, Day10.Pipe> pipes) ParseInput(string input)
    {
        var pipes = new Dictionary<Tuple<int, int>, Day10.Pipe>();
        ...
        for (var y = 0; y < rows.Length; y++)
        {
            for (var x = 0; x < rows[y].Length; x++)
            {
                if (rows[y][x] == 'S')
                {
                    startingPosition = new Tuple<int, int>(x, y);
                }
                else if (rows[y][x] != '.')
                {
                    pipes.Add(new Tuple<int, int>(x, y), ParsePipe(rows[y][x]));
                }
            }
        }

        return (startingPosition, pipes.ToImmutableDictionary());
    }

    protected override (Tuple<int, int> startingPosition, ImmutableDictionary<Tuple<int, int>, Day10.Pipe> pipes) ParseInputTwo(string input)
    {
        throw new NotImplementedException();
    }

    protected override long SolveFirstPuzzle(
        (Tuple<int, int> startingPosition, ImmutableDictionary<Tuple<int, int>, Day10.Pipe> pipes) input) =>
        Day10.firstPuzzle(input.startingPosition.Item1, input.startingPosition.Item2, input.pipes);

    protected override long SolveSecondPuzzle((Tuple<int, int> startingPosition, ImmutableDictionary<Tuple<int, int>, Day10.Pipe> pipes) input)
    {
        throw new NotImplementedException();
    }

    private static Day10.Pipe ParsePipe(char p) =>
        p switch
        {
            '|' => Day10.Pipe.NorthSouth,
            '-' => Day10.Pipe.EastWest,
            'L' => Day10.Pipe.NorthEast,
            'J' => Day10.Pipe.NorthWest,
            '7' => Day10.Pipe.SouthWest,
            _   => Day10.Pipe.SouthEast,
        };
