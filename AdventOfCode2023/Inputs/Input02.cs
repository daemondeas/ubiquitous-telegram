...
public class Input02 : AbstractInput<FSharpList<Day02.Game>, FSharpList<Day02.Game>>
{
    protected override FSharpList<Day02.Game> ParseInput(string input) => ListModule.OfSeq(input.Split('\n').Select(ParseGame));

    protected override FSharpList<Day02.Game> ParseInputTwo(string input) => ParseInput(input);

    protected override long SolveFirstPuzzle(FSharpList<Day02.Game> input) => Day02.firstPuzzle(input);

    protected override long SolveSecondPuzzle(FSharpList<Day02.Game> input) => Day02.secondPuzzle(input);

    private static Day02.Game ParseGame(string input)
    {
        var parts = input.Split(':');
        var id = int.Parse(parts[0].Split(' ')[1]);
        var rounds = parts[1].Split(';').Select(ParseRound);

        return new Day02.Game(id, ListModule.OfSeq(rounds));
    }

    private static Day02.Round ParseRound(string input)
    {
        ...
        foreach(var part in parts){
            ...
            if (innerParts[2] == "red")
            {
                red = int.Parse(innerParts[1]);
            }
            else if (innerParts[2] == "green")
            {
                green = int.Parse(innerParts[1]);
            }
            else
            {
                blue = int.Parse(innerParts[1]);
            }
        }

        return new Day02.Round(red, green, blue);
    }
...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green";
    ...
    public override string RealInput => @"Game 1: 2 red, 2 green; 6 red, 3 green; 2 red, 1 green, 2 blue; 1 red
